...
try:
    ...
    print(f'Python does not auto install. Download requirements.txt and star my project on GitHub if it helped!')
    input(f'Error: {e}')

color_webhook = 0xa80505
username_webhook = name_tool
avatar_webhook = 'https://media.discordapp.net'
...
def Reset():
    if os_name == "Windows":
        file = ['python', os.path.join(tool_path, "Jackpot.py")]
        subprocess.run(file)

    elif os_name == "Linux":
        file = ['python3', os.path.join(tool_path, "Jackpot.py")]
        subprocess.run(file)
...
def ChoiceUserAgent():
    file_user_agent = os.path.join(tool_path, "Imports", "Headers", "UserAgent.txt")
...
def ChoiceMultiTokenDisord():
    ...
    file_token_discord_relative = "\\Imports\TokenDisc\\TokenDisc.txt"
    file_token_discord = os.path.join(tool_path, "Imports", "TokenDisc", "TokenDisc.txt")
    tokens = {}
